"""
오케스트레이터: 멀티 에이전트 시스템을 조율하는 핵심 컴포넌트 (Ollama gemma4 버전)
"""
import os
...
class MultiAgentOrchestrator:
    """
    세 개의 에이전트를 조율하여 이광수 친일 챗봇의 응답을 생성 (Ollama gemma4)
    ...
    def __init__(self,
        ...
        Args:
            ...
            model_name: Ollama 모델 이름 (None이면 환경변수 OLLAMA_MODEL)
            embedding_model: 임베딩 모델 이름 (None이면 env GEMINI_EMBEDDING_MODEL — agents_2와 동일)
        """
        print("=" * 60)
        print("멀티 에이전트 시스템 초기화 중 (Ollama gemma4)...")
        print("=" * 60)
        ...
        if embedding_model is None:
            # 임베딩은 agents_2와 동일하게 Gemini 사용
            embedding_model = os.getenv("GEMINI_EMBEDDING_MODEL", "models/text-embedding-004")

        print(f"\n사용 모델(LLM): {model_name}")
        print(f"임베딩 모델(Gemini 유지): {embedding_model}\n")
    ...
    def chat(self):
        """대화형 인터페이스"""
        print("\n" + "="*60)
        print("이광수 친일 챗봇 (멀티 에이전트 버전 - Ollama gemma4)")
        print("="*60)
